...
public class PixelImage implements Cloneable {
	...
	public PixelImage(BufferedImage image, int height, char imgChar) {
		ChatColor[][] chatColors = toChatColorArray(image, height);
		lines = toImgMessage(chatColors, imgChar);
	}

	public PixelImage(ChatColor[][] chatColors, char imgChar) {
		lines = toImgMessage(chatColors, imgChar);
	}

	public PixelImage(String... imgLines) {
		lines = imgLines;
	}

	public PixelImage appendText(String... text) {
	...
	public PixelImage appendCenteredText(String... text) {
		for (int y = 0; y < lines.length; y++) {
			if (text.length > y) {
