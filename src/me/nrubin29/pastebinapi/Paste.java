...
public class Paste {
	...
	protected Paste(PastebinAPI api, ArrayList<String> args) throws PastebinException {
		...
		for (String arg : args) {
			...
			if (arg.startsWith("key")) {
				...
				date = new Date(Long.valueOf(sub(arg)));
			} else if (arg.startsWith("title")) {
				name = sub(arg);
			} else if (arg.startsWith("size")) {
				size = Integer.valueOf(sub(arg));
			} else if (arg.startsWith("expire_date")) {
				expiredate = new Date(Long.valueOf(sub(arg)));
			} else if (arg.startsWith("private")) {
				level = PrivacyLevel.valueOf(Integer.parseInt(sub(arg)));
			} else if (arg.startsWith("format_long")) {
				format = Format.valueOf(sub(arg));
	...
	public String[] getText() throws PastebinException {
		...
		try {
			return p.withURL(new URL("http://pastebin.com/raw.php?i=" + key)).post();
